import { IExecuteFunctions, ILoadOptionsFunctions, INodePropertyOptions, NodeOperationError, IDataObject } from 'n8n-workflow';
import {
	IAccount,
	IKommoUser,
	ICatalog,
...
export async function loadPipelinesOptions(
	this: IExecuteFunctions | ILoadOptionsFunctions,
): Promise<INodePropertyOptions[]> {
	const responseData = await apiRequest.call(this, 'GET', 'leads/pipelines', {}) as IDataObject;

	const embedded = responseData._embedded as IDataObject;
	if (!embedded?.pipelines) {
		throw new NodeOperationError(this.getNode(), 'No data got returned');
	}

	return (
		(embedded.pipelines as IPipeline[]).map(
